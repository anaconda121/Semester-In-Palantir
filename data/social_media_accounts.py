...
def social_media_accounts(n_accounts: int = 1_000, misinfo_rate: float = 0.07, seed: int = 2025) -> pd.DataFrame:
    ...
    # randomly assign primary platform (encoded as 0-4)
    primary_platform = rng.integers(0, 5, n_accounts, dtype=np.int8)

    # simulate account age in days using exponential distribution
    account_age_days = rng.exponential(scale=365 * 2, size=n_accounts).astype(np.int32)
    # simulate follower counts using lognormal distribution
    followers_cnt = rng.lognormal(mean=6, sigma=1.3, size=n_accounts).astype(np.int32)
    # 0/1 flag for verification status
    verified_flag = rng.integers(0, 2, size=n_accounts, dtype=np.int8)

    # base posting rate per day from gamma distribution
    base_post_rate = rng.gamma(shape=1.8, scale=0.7, size=n_accounts)

    # simulate prior flags and strikes with poisson distribution
    ...
    # assign misinformation label based on given misinfo rate
    label = rng.choice([0, 1], size=n_accounts, p=[1.0 - misinfo_rate, misinfo_rate]).astype(np.int8)

    # adjust distributions for misinfo accounts
    ...
    # simulate daily engagement metrics
